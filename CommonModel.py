...
class CommonModel():
	...
	#Custom model need this step
	image_path_ph = None
	image_data_resized_tensor = None
	#dropout
	keep_rate = 0.9
	learning_rate = None
	global_step = None
	...
	def __init__(self):
		print "Creating Common Model Base Class"
	...
	def _add_conv_layer(self,input,filterSizeX,filterSizeY,numInputChannels,numOutputChannels,enableLocalResponseNormalization,enableBatchNormalization,layerName,is_training):
	...
	def train_step(self,sess,trainX,trainY,dropout_keep_rate):
		return sess.run([self.train_step_op, self.cross_entropy_mean], feed_dict={self.x: trainX, self.y: trainY, self.keep_rate: dropout_keep_rate, self.is_training:True})

	def evaluate(self,sess,testX,testY):
		accuracy, cross_entropy_value = sess.run([self.evaluation_step,self.cross_entropy_mean],{self.x: testX, self.y: testY, self.keep_rate:1, self.is_training:False})
	...
	#Custom Model Needs to resize images to same size
	def add_image_standardization_ops(self,FLAGS):
		 with self.graph.as_default():
			 self.image_data_ph = tf.placeholder('float', [None,None,MODEL_INPUT_DEPTH])
			 #image_data = tf.image.decode_image(self.image_path_ph)
			 #image_data = tf.image.decode_png(self.image_path_ph)
			 self.image_data_resized_tensor = tf.image.resize_image_with_crop_or_pad(self.image_data_ph,IMAGE_SIZE_X,IMAGE_SIZE_Y)

	def standardize_image(self,sess,image_data):
		return sess.run([self.image_data_resized_tensor],feed_dict={self.image_data_ph:image_data})
